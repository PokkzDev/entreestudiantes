...
export function useSubscription() {
  ...
  // Note: Tier information should be fetched from database when needed
  // No longer using hardcoded ACCOUNT_TIERS
  ...
  const hasFeature = (feature) => {
    // Features should be checked against database tier data
    console.warn('hasFeature function requires database tier data');
    return false;
  };
  
  const canCreatePublication = (currentCount) => {
    // Publication limits should be checked against database tier data
    console.warn('canCreatePublication function requires database tier data');
    return {
      canCreate: true,
      limit: null,
      remaining: null,
  ...
  return {
    ...
    // Note: currentTier and originalTier removed - should be fetched from database

...
/**
 * Verify MercadoPago webhook signature según documentación oficial
...
export function verifyWebhookSignature(xSignature, xRequestId, dataId, secret) {
  try {
    ...
    // Validate signature format according to MercadoLibre documentation
    if (!xSignature || !xSignature.includes('ts=') || !xSignature.includes('v1=')) {
      console.error('❌ Invalid signature format - missing ts= or v1=');
      return false;
    }
    
    // Parse signature components according to documentation
    const parts = xSignature.split(',');
    let ts = null;
    let hash = null;
    
    // Extract ts and v1 values
    parts.forEach(part => {
    ...
    if (!ts || !hash) {
      console.error('❌ Could not extract timestamp or signature from x-signature header');
      return false;
    }
    ...
    // Create the manifest string according to MercadoLibre documentation
    // Format: id:[data.id_url];request-id:[x-request-id_header];ts:[ts_header];

...
import { validateMercadoPagoSetup } from './mercadopago-config.js';

// Validate configuration on import
const mpConfig = validateMercadoPagoSetup();

// Check if we're in test environment
const isTestEnvironment = mpConfig.environment === 'test';
...
export async function createSubscriptionPreference(planData, userData) {
  try {
    // Use test RUT only in test environment, otherwise require valid RUT
    const userRut = isTestEnvironment ? '188083986' : (userData.rut || '');
    
    if (!isTestEnvironment && !userRut) {
      throw new Error('RUT is required for production payments');
    }
    ...
    console.log(`Creating preference in ${isTestEnvironment ? 'TEST' : 'PRODUCTION'} mode`);
    
    const response = await preference.create({ body: preferenceData });
    
    // Return the appropriate init_point based on environment
    const initPoint = isTestEnvironment ? response.sandbox_init_point : response.init_point;
...
export async function verifyPayment(paymentId) {
  try {
    ...
    console.log(`Using ${isTestEnvironment ? 'sandbox' : 'production'} environment`);
    ...
    console.log(`MercadoPago API response status: ${payment.status}`);
    
    if (!payment.ok) {
      const errorBody = await payment.text();
      console.error(`MercadoPago API error: ${payment.status} - ${errorBody}`);
      
      if (payment.status === 404) {
        throw new Error(`Payment not found. Payment ID: ${paymentId}. Check if the payment exists and if you're using the correct environment (${isTestEnvironment ? 'sandbox' : 'production'}).`);
      } else if (payment.status === 401) {
        throw new Error(`Unauthorized. Check your MercadoPago access token.`);
      } else {
        throw new Error(`Payment verification failed: ${payment.status} - ${errorBody}`);
      }
    }

...
export async function verifyPayment(paymentId) {
  try {
    ...
    const isTest = process.env.MERCADOPAGO_ACCESS_TOKEN?.includes('TEST');
    
    console.log(`Verifying payment ID: ${paymentId}`);
    console.log(`Using ${isTest ? 'sandbox' : 'production'} environment`);
    ...
    if (!payment.ok) {
      ...
      if (payment.status === 404) {
        throw new Error(`Payment not found. Payment ID: ${paymentId}. Check if the payment exists and if you're using the correct environment (sandbox/production).`);
